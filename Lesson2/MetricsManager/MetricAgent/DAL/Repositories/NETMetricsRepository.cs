...
namespace MetricAgent.DAL.Repositories
{
	...
	/// </summary>
	public interface INETMetricsRepository : IRepository<Metric>
	{
	}
	...
	public class NETMetricsRepository : INETMetricsRepository
	{
		...
		public IList<Metric> GetByTimePeriod(DateTimeOffset fromTime, DateTimeOffset toTime)
		{
			...
			using var cmd = new SQLiteCommand(connection);
			cmd.CommandText = "SELECT * FROM netmetrics WHERE ({mySql[Columns.Time]} >= @fromTime AND {mySql[Columns.Time]} <= @toTime)";
